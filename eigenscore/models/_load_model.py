...
def _load_pretrained_model(model_name, device, torch_dtype=torch.float16):
    if model_name.startswith('facebook/opt'):
        local_path = os.path.join(MODEL_PATH, model_name.split("/")[1])
        if os.path.exists(local_path):
            model = OPTForCausalLM.from_pretrained(local_path, torch_dtype=torch_dtype)
        else:
            print(f"로컬에 모델이 없습니다. Hugging Face에서 {model_name}을 다운로드합니다...")
    ...
    if model_name == 'llama-7b-hf' or model_name == 'llama-13b-hf' or model_name == "llama2-7b-hf":
        # 로컬 경로 먼저 확인, 없으면 Hugging Face에서 다운로드
        local_path = os.path.join(MODEL_PATH, model_name)
        if os.path.exists(local_path):
            model = AutoModelForCausalLM.from_pretrained(local_path, cache_dir=None, torch_dtype=torch_dtype)
        else:
            # Hugging Face Hub에서 다운로드 (meta-llama/Llama-2-7b-hf 사용)
            if model_name == "llama-7b-hf":
                ...
                hf_model_name = "meta-llama/Llama-2-7b-hf"
            print(f"로컬에 모델이 없습니다. Hugging Face에서 {hf_model_name}을 다운로드합니다...")
...
def _load_pretrained_tokenizer(model_name, use_fast=False):
    if model_name.startswith('facebook/opt-'):
        local_path = os.path.join(MODEL_PATH, model_name.split("/")[1])
        if os.path.exists(local_path):
            tokenizer = AutoTokenizer.from_pretrained(local_path, use_fast=use_fast)
        else:
        ...
        local_path = os.path.join(MODEL_PATH, model_name)
        if os.path.exists(local_path):
            tokenizer = AutoTokenizer.from_pretrained(local_path, cache_dir=None, use_fast=use_fast)
        else:
            if model_name == "llama-7b-hf":
                hf_model_name = "meta-llama/Llama-2-7b-hf"
            elif model_name == "llama-13b-hf":
